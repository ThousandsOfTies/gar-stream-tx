...
を使う。KY-040 ロータリーエンコーダで、送信する **解像度（サイズ）** と
**フレームレート（レート）** をその場で切り替えられるようにする。

- **回す** → `SIZE_PRESETS` を順に切り替え（送信解像度）
- **押す** → `RATE_PRESETS` を順に切り替え（送信フレームレート）

加えて、**TX 自身に ILI9341 を直結してローカルプレビューを出す**ことも
できる（`CONFIG["local_display"] = True`）。カメラ入力を RX に送るのとは
別に、同じデコード済みフレームを TX 上の SPI パネルにもそのまま描画できる
— 詳しくは「アーキテクチャ」と「1. 配線」を参照。ディスプレイを繋がない
場合は `camera_tx.py` の標準出力ログで現在の解像度/レートを確認する。
...
DEFAULT_SIZE_INDEX = 1   # 640x480

RATE_PRESETS = [5, 10, 15]
DEFAULT_RATE_INDEX = 2   # 15fps
```

いずれも 4:3 に揃えている。RX 側の ILI9341 パネルが 320×240（4:3）なので、
レターボックス無しで表示できる。既定値の 640×480 はパネル解像度のちょうど
2倍で、[gar-stream-rx/README.md](../gar-stream-rx/README.md) の SPI 帯域幅
の項が推奨するサイズと一致させてある。必要に応じて `camera_tx.py` の
`SIZE_PRESETS`/`RATE_PRESETS` を編集して構わない。
...
起動ログに現在の `WxH@Rfps -> host:port` が出る。KY-040 を回すとサイズが、
押すとレートが切り替わり、そのたびにログが出る。ネットワーク分岐の
`capsfilter` を差し替えているだけでパイプライン自体は止めていないので、
切り替え中に RX 側の接続が切れたりカメラを再オープンしたりはしない
（数フレーム分の乱れはあり得る）。
